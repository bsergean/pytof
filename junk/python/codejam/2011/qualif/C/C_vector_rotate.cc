...
int compute(vector<uint>& candies, uint L)
{
    int winner = -1;
    for (uint z = 0; z < L; ++z) {
        for (uint i = 0; i < (L-1); ++i) {
            ...
            vector<uint>::const_iterator it  = candies.begin();
            vector<uint>::const_iterator end = candies.end();
